...
public class SqlGenerator {
        ...
        if(beanMap!=null){
            ...
            if(orderBy!=null) {
                ...
                for (OrderBy.OrderByInfo orderByInfo : orders) {
                    ...
                    sbOrderBy.append(colName)
                            .append(" ")
                            .append(orderByInfo.direction)
                            .append(",");
                }
                //删除最后的,号
                if(sbOrderBy.length()>0){
     ...
     * @return insert语句
     */
    public static <T> String genInsertStatement(T bean){
        if(bean==null){
            return "";
        }
        SqlMapper.BeanMap beanMap=SqlMapper.getBeanMap(bean.getClass());
        StringBuilder sbColumns=new StringBuilder();
        StringBuilder sbValues=new StringBuilder();
        //取得实体对象的所有属性值
        Map<String,Object> valueMap= ReflectUtils.beanToMap(bean);
        for(SqlMapper.ColField cf : beanMap.fields){
            //过滤属性值为空的字段
            if (valueMap.get(cf.propName)!=null) {
                //a,b,c
        ...
        //去掉最后的","
        int lastIndex=sbColumns.lastIndexOf(",");
        if(lastIndex==sbColumns.length()-1){
            sbColumns.deleteCharAt(lastIndex);
        }
        lastIndex=sbValues.lastIndexOf(",");
        if(lastIndex==sbValues.length()-1){
            sbValues.deleteCharAt(lastIndex);
        }
