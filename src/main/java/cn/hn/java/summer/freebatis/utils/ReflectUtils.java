...
public class ReflectUtils extends ReflectionUtils{
	...
	public static boolean hasProperty(Class cls){
		...
		if(p==null || p.getName().indexOf("java.lang")==0){
			return false;
		}
	...
	public static void setFieldValue(Object o, String name, Object value){
		...
		try {
			...
			Class[] paraTypes = setter.getParameterTypes();
			if(paraTypes.length==0){
				return;
			}
			/*
			如果参数类型是Date且value是long则将value转为date进行设置
			以解决date类型序列化后反序列化的问题
			 */
			if(paraTypes[0] == Date.class && value!=null && value instanceof Long){
